...
export async function onRequestPut(context) {
  ...
  try {
    ...
    const result = await db.prepare(`UPDATE store_products SET name=?,description=?,level=?,brand=?,category=?,price_satang=?,cost_price_satang=?,
      stock_quantity=?,image_url=?,status=?,sort_order=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`)
      .bind(product.name,product.description,product.level,product.brand,product.category,product.priceSatang,product.costPriceSatang,product.stockQuantity,
