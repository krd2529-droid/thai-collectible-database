...
function beginCheckout(product,quantity){if(!product||!Number.isInteger(quantity)||quantity<1||quantity>product.availableStock)return message('จำนวนสินค้าไม่ถูกต้อง','error');selected={product,quantity};clientToken=crypto.randomUUID();$('checkoutSummary').innerHTML=`<b>${esc(product.name)}</b><br>จำนวน ${quantity} × ${money(product.price)}<br><strong>ยอดรวม ${money(product.price*quantity)}</strong>`;$('createOrder').disabled=false;$('createOrder').classList.remove('hidden');$('facebookButton').classList.add('hidden');checkoutMessage();$('checkoutDialog').showModal()}
function render(products) {
  ...
  $('products').querySelectorAll('[data-buy]').forEach((button)=>button.addEventListener('click',()=>{const product=products.find((item)=>item.id===button.dataset.buy);beginCheckout(product,Number($(`qty-${product.id}`).value))}));
  const directId=new URLSearchParams(location.search).get('product');
  if(directId){const directProduct=products.find(item=>item.id===directId);const directQuantity=Math.max(1,Number(new URLSearchParams(location.search).get('quantity'))||1);if(directProduct&&directProduct.availableStock>0&&directProduct.status!=='sold_out')beginCheckout(directProduct,directQuantity)}
}
async function load(){
