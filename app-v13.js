console.info("TOYSKUB app-v13 loaded");
...
function renderDetail(p) {
  ...
  // V13: force-visible catalog actions. Uses inline styles so stale CSS cannot hide it.
  const forcedActions = document.createElement("div");
  forcedActions.id = "catalogAdminActionsV13";
  forcedActions.setAttribute("data-version", "v13");
  forcedActions.style.cssText = "display:flex!important;flex-wrap:wrap;gap:8px;align-items:center;margin:12px 0 16px;padding:12px;background:#e7edf5;border:2px solid #101820;position:relative;z-index:50;font-family:JetBrains Mono,monospace";
  forcedActions.innerHTML = `
    <a href="/admin/rg-template/" style="display:inline-flex;padding:9px 13px;border:1px solid #101820;background:#fff;color:#101820;font-weight:700">+ เพิ่มหน้าใหม่</a>
    <a href="/admin/rg-template/?id=${encodeURIComponent(p.id)}" style="display:inline-flex;padding:9px 13px;border:1px solid #101820;background:#244f8d;color:#fff;font-weight:700">แก้ไข / เพิ่มข้อมูล</a>
    <button type="button" data-v13-move style="padding:9px 13px;border:1px solid #101820;background:#fff;color:#101820;font-weight:700;cursor:pointer">ย้าย / จัดลำดับ</button>
    <button type="button" data-v13-trash style="padding:9px 13px;border:1px solid #a52020;background:#fff0f0;color:#a52020;font-weight:700;cursor:pointer">ลบ / ถังขยะ</button>
  `;
  const oldActions = APP.querySelector(".catalog-admin-actions");
  if (oldActions) oldActions.replaceWith(forcedActions);
  else APP.prepend(forcedActions);
  forcedActions.querySelector("[data-v13-move]")?.addEventListener("click", () => openFrontendMoveDialog(p));
  forcedActions.querySelector("[data-v13-trash]")?.addEventListener("click", () => {
    document.getElementById("catalogTrashButton")?.click();
  });
