...
| TOY-F015 | Multi-category storefront | ทางเข้าอยู่บน `/`, หมวดการ์ด `/stock/onepiececard/`, หมวดของเล่น `/stock/toys/` และหน้าสินค้าใต้หมวดนั้น | `GET /api/store/products?category=...`, `POST /api/store/orders` | D1 `store_products`, `store_orders` (ไม่ใช้ catalog JSON/`catalog_items`) | category, product, quantity, shipping input → pending order reference | สินค้าในร้านแยกจากแคตตาล็อก; สินค้าเดิมคงหมวดการ์ดวันพีช; stock 0 = Sold out |
