...
| TOY-F015 | One Piece Card storefront | ทางเข้า Store แยกบน `/` และหน้าร้าน `/shop/`, `shop/shop.js` | `GET /api/store/products`, `POST /api/store/orders` | D1 `store_products`, `store_orders` (ไม่ใช้ catalog JSON/`catalog_items`) | product, quantity, shipping input → pending order reference | ราคา/ชื่อถูก snapshot; public response ไม่คืนข้อมูลจัดส่ง; stock 0 = Sold out |
