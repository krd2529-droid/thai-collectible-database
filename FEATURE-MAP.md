...
|---|---|---|---|---|---|---|
| TOY-F001 | Public catalog navigation | `index.html`, `app.js:loadData/renderHome` | `GET /api/catalog` | static category/index JSON + D1 `catalog_items` | category/line selection → catalog cards | SQL อ่านเฉพาะ published payload และ compact excluded IDs; public response cache 60 วินาที; ห้ามเปลี่ยน precedence static/D1 โดยไม่มี contract test |
| TOY-F002 | Product detail | `/product/:id`, `app.js:route/renderDetail` | `GET /api/catalog/:id` สำหรับ D1 | detail JSON หรือ D1 payload | product ID → full product page | อ่านด้วย id + published status โดยตรงและ cache 5 นาที; `_redirects`, SEO URL, missing/hidden status |
| TOY-F003 | Traffic analytics | `app.js:recordPageView/loadVisitorStats` | `POST /api/analytics/view`, `GET /api/analytics/stats` | D1 `analytics_daily`, `analytics_totals`; sessionStorage ป้องกันการนับซ้ำใน session | page view → today/7/30/total และยอดต่อหน้า | ไม่เก็บ event ใหม่; Migration 0008 สรุปข้อมูลเดิมโดยยังไม่ลบ `analytics_views`; หากยังไม่ได้รัน Migration ระบบ provision เฉพาะเมื่อพบ missing table ครั้งแรก; cache สถิติหนึ่งครั้งต่อหน้า |
| TOY-F004 | Admin shared-password session | `/admin/`, `admin/admin.js` | login/logout/session | `ADMIN_PASSWORD`, `ADMIN_EMAIL`, signed cookie, D1 users | password → admin cookie/session state | ตรวจ member token ก่อนแตะ D1 และไม่ provision schema ใน session read; high-risk auth |
...
| TOY-F007 | Catalog manager | `/admin/catalog/` | admin catalog list/get/delete/bulk gallery APIs | D1 `catalog_items` | filters/actions → catalog state | list API ใช้ `json_extract` คืนเฉพาะ gallery flag แทน payload เต็ม; status semantics และ static tombstone behavior |
...
| TOY-F015 | Multi-category storefront | ทางเข้าอยู่บน `/`, หมวดการ์ด `/stock/onepiececard/`, หมวดของเล่น `/stock/toys/` และหน้าสินค้าใต้หมวดนั้น | `GET /api/store/products?category=...`, `POST /api/store/orders` | D1 `store_products`, `store_orders` (ไม่ใช้ catalog JSON/`catalog_items`) | category, product, quantity, shipping input → pending order reference | public read ไม่รัน schema DDL/probe และ cache สั้น 15–30 วินาที; สินค้าเดิมคงหมวดการ์ดวันพีช; stock 0 = Sold out |
| TOY-F016 | Store administration | `/admin/store/` พร้อมตัวเลือกหมวดและลิงก์ตรวจหน้ารวม/หน้าสินค้าตามหมวด | admin store product/order APIs + media upload | D1 + R2 `TOYSKUB_MEDIA` | product CRUD-like update (รวมหมวด ยี่ห้อ ระดับ ราคาต้นทุน และ public URL), optimized image, order transition → inventory state | admin GET ไม่รัน schema DDL/probe; ราคาต้นทุนส่งเฉพาะ admin API; paid เท่านั้นที่หัก stock |
| TOY-F017 | Auto Catalog RG | `/admin/rg-template/` → Auto Catalog Draft | `POST /api/admin/ai/catalog-fill`, `POST /api/admin/media/import-dalong-manual`, catalog admin API | D1 `catalog_items` + R2 `TOYSKUB_MEDIA` | ชื่อ/รหัสรุ่น → ข้อมูลอ้างอิง + Dalong Information URL → รูปปก + รูปคู่มือ → Draft | รองรับเฉพาะ Dalong URL ที่ตรวจ host; ไม่เดาลิงก์; ไม่ Publish อัตโนมัติ; ผู้ดูแลต้องตรวจ Draft |
| TOY-F018 | Full MGSD editor + Auto Catalog | `/admin/mgsd-template/` | AI catalog fill, Dalong import, catalog admin APIs | D1 `catalog_items` + static MGSD fallback + R2 | ชื่อ/รหัส MGSD → ข้อมูล, ปก, คู่มือ → Draft/Publish หลังตรวจ | ใช้ฟอร์มแกนเดียวกับ RG แต่แยก line, number, scale, path และ AI rules; Auto Catalog ไม่ Publish เอง |
