...
| TOY-F015 | One Piece Card storefront | การ์ดสินค้าบน `/`, หน้าสินค้าและ checkout `/stock/onepiececard/{product-id}`, `app.js`, `index.html` | `GET /api/store/products`, `POST /api/store/orders` | D1 `store_products`, `store_orders` (ไม่ใช้ catalog JSON/`catalog_items`) | product, quantity, shipping input → pending order reference | แยกจากแคตตาล็อก; ราคา/ชื่อถูก snapshot; stock 0 = Sold out |
