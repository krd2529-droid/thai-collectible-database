...
async function compressImage(file,max=1100,quality=.78){return new Promise((resolve,reject)=>{const img=new Image(),url=URL.createObjectURL(file);img.onload=()=>{let w=img.width,h=img.height;if(Math.max(w,h)>max){const r=max/Math.max(w,h);w=Math.round(w*r);h=Math.round(h*r)}const c=document.createElement('canvas');c.width=w;c.height=h;c.getContext('2d').drawImage(img,0,0,w,h);URL.revokeObjectURL(url);c.toBlob(blob=>blob?resolve(new File([blob],file.name.replace(/\.[^.]+$/,'.webp'),{type:'image/webp'})):reject(new Error('แปลงรูปไม่สำเร็จ')),'image/webp',quality)};img.onerror=()=>reject(new Error('อ่านไฟล์รูปไม่สำเร็จ'));img.src=url})}
async function uploadMediaFile(file,target){const id=val('itemId').toLowerCase();if(!id)throw new Error('กรุณากรอกรหัสรายการก่อนอัปโหลดรูป');const compressed=await compressImage(file,target==='manual'?1600:1200,target==='manual'?.82:.78);const form=new FormData();form.append('file',compressed);form.append('id',id);form.append('kind',target);const r=await fetch('/api/admin/media/upload',{method:'POST',body:form,cache:'no-store'});const d=await r.json().catch(()=>({}));if(r.status===401){location.href='/admin/';throw new Error('กรุณาเข้าสู่ระบบใหม่')}if(!r.ok)throw new Error(d.error||'อัปโหลดรูปไม่สำเร็จ');return {src:d.url,name:d.name||compressed.name}}
